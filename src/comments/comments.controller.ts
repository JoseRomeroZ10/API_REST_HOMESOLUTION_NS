...
export class CommentsController {
  ...
  @UseGuards(AuthGuard)
  create(@Param('rentalId', ParseIntPipe)rentalId: number, @Body() createCommentDto: CreateCommentDto, @ActiveUser() user: UserActiveInterface) {
    return this.commentsService.create(createCommentDto, user, rentalId);
  }

  @Get()
  findAll( createCommentDto: CreateCommentDto, @ActiveUser() user: UserActiveInterface) {
  ...
  findOne(@Param('id') id: number, @ActiveUser() user: UserActiveInterface) {
   ...
   @Patch(':id/rentals/:rental_id')
    @UseGuards(AuthGuard)
   update(@Param('id', ParseIntPipe) id: number,@Param('rental_id', ParseIntPipe) rental_id: number, @Body() updateCommentDto: UpdateCommentDto, @ActiveUser() user: UserActiveInterface) {
     return this.commentsService.update(id, updateCommentDto, user, rental_id);
   }

  @Delete(':id/rentals/:rental_id')
  @UseGuards(AuthGuard)
  remove(@Param('id', ParseIntPipe) id: number,@Param('rental_id', ParseIntPipe) rental_id: number, @ActiveUser() user: UserActiveInterface) {
